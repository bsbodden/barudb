...
impl CompactionPolicy for LazyLeveledCompactionPolicy {
    ...
    fn compact(
        ...
        for run in &source_runs {
            ...
            if std::env::var("RUST_LOG").map(|v| v == "debug").unwrap_or(false) {
                ...
                if tombstone_count > 0 {
                    println!("Found {} tombstones in source run during lazy-leveled compaction", tombstone_count);
                }
            }
        }
        ...
        for run in &target_runs {
            ...
            if std::env::var("RUST_LOG").map(|v| v == "debug").unwrap_or(false) {
                let tombstone_count = run.data.iter()
                    ...
                    .count();
                if tombstone_count > 0 {
                    println!("Found {} tombstones in target run during lazy-leveled compaction", tombstone_count);
                }
            }
        }
        ...
        if std::env::var("RUST_LOG").map(|v| v == "debug").unwrap_or(false) {
            ...
            println!("Lazy-leveled compaction - After deduplication: {} tombstones remain", tombstone_count_after);
            
            if tombstone_count_after > 0 {
                for (key, value) in &all_data {
                    if *value == crate::types::TOMBSTONE {
                        println!("Lazy-leveled compaction - Merged data contains tombstone for key: {}", key);
                    }
                }
            }
        }
